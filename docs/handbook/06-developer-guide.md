...
metaphor migration generate Invoice approvals
metaphor migration run
...
(`approvals` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed approvals          # run Rust seeders
metaphor migration generate-seeds approvals  # emit SQL seed files
```
