//! Domain policy for ApprovalRequest aggregate
//!
...
/// Domain policy for ApprovalRequest — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ApprovalRequest>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ApprovalRequestDomainPolicy = PermitAllPolicy<ApprovalRequest>;
