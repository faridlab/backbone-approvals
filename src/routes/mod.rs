...
/// Create stateless routes for the Approvals module.
///
...
///
/// ```ignore
/// let module = ApprovalsModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::ApprovalsModule) -> Router<()> {
    Router::new()
        .merge(create_approval_policy_routes(module.approval_policy_service.clone()))
        .merge(create_approval_request_routes(module.approval_request_service.clone()))
        .merge(create_approval_step_routes(module.approval_step_service.clone()))
        .merge(create_approval_step_template_routes(module.approval_step_template_service.clone()))
        .merge(create_delegation_routes(module.delegation_service.clone()))
}

/// Read-only routes for the Approvals module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_approvals_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_approvals_routes(module: &crate::ApprovalsModule) -> Router<()> {
    Router::new()
        .merge(create_approval_policy_read_routes(module.approval_policy_service.clone()))
        .merge(create_approval_request_read_routes(module.approval_request_service.clone()))
        .merge(create_approval_step_read_routes(module.approval_step_service.clone()))
        .merge(create_approval_step_template_read_routes(module.approval_step_template_service.clone()))
        .merge(create_delegation_read_routes(module.delegation_service.clone()))
}

/// Get all routes (stateless) for the Approvals module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::ApprovalsModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Approvals module.
///
...
pub fn create_combined_routes(module: &crate::ApprovalsModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Approvals module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::ApprovalsModule) -> Router<AppState> {
    Router::new()
