//! Application service for ApprovalPolicy entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `approval_policy_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateApprovalPolicyDto, UpdateApprovalPolicyDto};
use crate::domain::entity::ApprovalPolicy;
use crate::infrastructure::persistence::ApprovalPolicyRepository;

/// Application service for ApprovalPolicy entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `approval_policy_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ApprovalPolicyServiceCustom {
///     inner: Arc<ApprovalPolicyService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ApprovalPolicyService = GenericCrudService<
    ApprovalPolicy,
    CreateApprovalPolicyDto,
    UpdateApprovalPolicyDto,
    ApprovalPolicyRepository,
>;

// <<< CUSTOM
// END CUSTOM
