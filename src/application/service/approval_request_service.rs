//! Application service for ApprovalRequest entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `approval_request_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateApprovalRequestDto, UpdateApprovalRequestDto};
use crate::domain::entity::ApprovalRequest;
use crate::infrastructure::persistence::ApprovalRequestRepository;

/// Application service for ApprovalRequest entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `approval_request_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ApprovalRequestServiceCustom {
///     inner: Arc<ApprovalRequestService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ApprovalRequestService = GenericCrudService<
    ApprovalRequest,
    CreateApprovalRequestDto,
    UpdateApprovalRequestDto,
    ApprovalRequestRepository,
>;

// <<< CUSTOM
// END CUSTOM
