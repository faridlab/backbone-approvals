...
//! host uses) plus route level behind the org session context (`OrgContext`) the
//! composing service's `org_auth` middleware inserts.
//!
//! What the suite pins:
...
//!   step_no).
//!
//! Tenancy posture (ADR-0029): the module carries no tenancy of its own — the row-level
//! fence and the org-scoped uniques (one live request per resource, one policy name per
//! resource family, one active policy per resource) arrive with the composing service's
//! tenancy decorator, so no probe asserts cross-principal isolation module-side. Each
//! engine transaction relays the AMBIENT org scope when the caller bound one; the probes
//! below run undecorated on a scratch database — a posture only a test bench allows,
//! since a composing service always installs the decorator.
//!
//! DB: DATABASE_URL wins, else the module's local test DB (`backbone_approvals_test` on
//! the metaphora dev postgres, migrated). Fresh random ids per test so parallel runs
//! never collide.
...
/// The engine files against THE active policy of a resource type. With the company
/// predicate gone from module SQL, probes sharing one scratch database and one enum
/// value would race each other's policies; this guard serializes the suite and each
/// probe retires whatever an earlier one left active, so every filing sees exactly
/// its own policy.
static SEQUENTIAL: Mutex<()> = Mutex::new(());

async fn pool() -> (MutexGuard<'static, ()>, PgPool) {
    let sequential = SEQUENTIAL.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
/// Retire the active policies of this resource type left by earlier probes: the module
/// files against the one active policy per type, so a leftover would hijack a filing.
async fn scrub_active_policies(pool: &PgPool, resource_type: &str) {
    sqlx::query(
        r#"UPDATE approvals.approval_policies SET status = 'inactive'
             WHERE resource_type = $1::approval_resource_type AND status = 'active'"#,
    )
    .bind(resource_type)
    .execute(pool)
    .await
...
/// Bind a throwaway company-unit scope around engine verbs whose dynamic approver
/// resolution keys on the legacy company seam during the tenancy transition: without
/// one the resolution fails closed on the missing scope before the resolver is even
/// consulted. A composing host binds the session's resolved scope; the scratch bench
/// binds a random unit, which is all the seam reads.
...
/// The acting principal as an `OrgContext` — the identity currency the guarded handlers
/// extract. The composing service's REAL path (signed org token via its `org_auth`
/// middleware over the tenant router) resolves the session's org scope from the org
/// spine and is proven in that service's suite; this scratch database has no spine, so
/// the probes insert the extension directly.
fn org_principal(sub: Uuid) -> OrgContext {
    OrgContext {
...
async fn req(
    ...
    let mut r = Request::builder()
        ...
        .body(Body::from(body))
        .unwrap();
    r.extensions_mut().insert(org_principal(actor));
...
async fn seed_policy(pool: &PgPool, resource_type: &str) -> Uuid {
    scrub_active_policies(pool, resource_type).await;
    let id = Uuid::new_v4();
    sqlx::query(
        r#"INSERT INTO approvals.approval_policies
               (id, resource_type, name, status, description, metadata)
           VALUES ($1, $2::approval_resource_type, $3, 'active', NULL, '{}'::jsonb)"#,
    )
    .bind(id)
    .bind(resource_type)
    .bind("probe policy")
...
async fn seed_template(
    ...
    sqlx::query(
        r#"INSERT INTO approvals.approval_step_templates
               (id, policy_id, step_no, approver_kind, approver_ref, sla_hours,
                all_of, metadata)
           VALUES ($1, $2, $3, $4::approver_kind, $5, NULL, $6, '{}'::jsonb)"#,
    )
    .bind(id)
    .bind(policy)
    .bind(step_no)
...
fn filing(resource: Uuid, requester: Uuid) -> FileFiling {
    FileFiling {
...
fn decide_as(request: Uuid, step_no: i32, actor: Uuid, approve: bool) -> Decision {
    Decision {
        request_id: request,
        step_no,
...
/// The live step row of one assignee, for assertions.
async fn step_row(pool: &PgPool, request: Uuid, assigned_to: Uuid) -> (String, Option<Uuid>) {
    let row: (String, Option<Uuid>) = sqlx::query_as(
        r#"SELECT status::text, delegated_from FROM approvals.approval_steps
            WHERE request_id = $1 AND assigned_to = $2
...
async fn count_steps(pool: &PgPool, request: Uuid) -> i64 {
    sqlx::query_scalar(
...
async fn no_policy_files_pre_approved_with_zero_steps() {
    ...
    // An earlier probe's policy must not hijack this filing's no-policy posture.
    scrub_active_policies(&pool, "leave").await;
    let svc = engine(&pool);

    // No policy seeded for this resource type.
    let outcome = svc.file(filing(resource, employee)).await.unwrap();
    assert_eq!(outcome.verdict, ApprovalStatus::Approved);
    assert!(!outcome.already_filed);
    assert_eq!(count_steps(&pool, outcome.request_id).await, 0);
    ...
    assert_eq!(
        svc.status(outcome.request_id).await.unwrap(),
...
async fn policy_materializes_every_step_once() {
    ...
    seed_template(&pool, policy, 1, "specific_employee", Some(approver1), None).await;
    seed_template(&pool, policy, 2, "specific_employee", Some(approver2), None).await;
    let svc = engine(&pool);

    let resource = Uuid::new_v4();
    let outcome = svc.file(filing(resource, employee)).await.unwrap();
    assert_eq!(outcome.verdict, ApprovalStatus::Pending);
    assert_eq!(count_steps(&pool, outcome.request_id).await, 2);

    let request = svc.get_request(outcome.request_id).await.unwrap();
    assert_eq!(request.current_step, Some(1));

    // Step 2 exists from filing but the chain has not reached it.
    assert_eq!(
        step_row(&pool, outcome.request_id, approver2).await.0,
        "pending"
    );
}
...
async fn all_of_quorum_requires_every_member() {
    ...
    let outcome = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();
    // One row per member, all at step 1.
    assert_eq!(count_steps(&pool, outcome.request_id).await, 3);

    svc.decide(decide_as(outcome.request_id, 1, a, true))
        .await
        .unwrap();
    assert_eq!(
        svc.status(outcome.request_id).await.unwrap(),
    ...
    assert_eq!(step_row(&pool, outcome.request_id, b).await.0, "pending");

    svc.decide(decide_as(outcome.request_id, 1, b, true))
        .await
        .unwrap();
    assert_eq!(
        svc.status(outcome.request_id).await.unwrap(),
    ...
    svc.decide(decide_as(outcome.request_id, 1, c, true))
        .await
        .unwrap();
    assert_eq!(
        svc.status(outcome.request_id).await.unwrap(),
    ...
    assert_eq!(step_row(&pool, outcome.request_id, a).await.0, "approved");
    assert_eq!(step_row(&pool, outcome.request_id, b).await.0, "approved");
}
...
async fn reject_fails_fast_and_skips_siblings() {
    ...
    .await;
    seed_template(&pool, policy, 2, "specific_employee", Some(step2), None).await;
    let svc = engine(&pool);

    let outcome = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();

    let verdict = svc
        .decide(decide_as(outcome.request_id, 1, a, false))
        .await
        .unwrap();
    ...
    assert_eq!(
        step_row(&pool, outcome.request_id, a).await.0,
        "rejected"
    );
    assert_eq!(
        step_row(&pool, outcome.request_id, b).await.0,
        "skipped"
    );
    assert_eq!(
        step_row(&pool, outcome.request_id, step2).await.0,
        "skipped"
    );

    // A late decide on the corpse is a 409, not a state change.
    let err = svc
        .decide(decide_as(outcome.request_id, 1, b, true))
        .await
        .unwrap_err();
...
async fn step_two_waits_for_step_one() {
    ...
    let err = svc
        .decide(decide_as(outcome.request_id, 2, approver2, true))
        .await
        .unwrap_err();
    assert!(matches!(err, ApprovalsError::OutOfTurn));

    svc.decide(decide_as(outcome.request_id, 1, approver1, true))
        .await
        .unwrap();
    let request = svc.get_request(outcome.request_id).await.unwrap();
...
async fn live_delegation_pre_resolves_at_file_time() {
    ...
    .await;
    let outcome = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();
    let (status, delegated_from) = step_row(&pool, outcome.request_id, approver).await;
    ...
    let err = svc
        .decide(decide_as(outcome.request_id, 1, delegate, true))
        .await
        .unwrap_err();
    ...
    .await;
    let outcome2 = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();
    let (status, delegated_from) = step_row(&pool, outcome2.request_id, delegate).await;
    assert_eq!(status, "pending");
    assert_eq!(delegated_from, Some(approver));

    // The delegate decides; the stamp records whose authority was used (not the decider).
    let verdict = svc
        .decide(decide_as(outcome2.request_id, 1, delegate, true))
        .await
        .unwrap();
    assert_eq!(verdict, ApprovalStatus::Approved);
    let (_, delegated_from) = step_row(&pool, outcome2.request_id, delegate).await;
    assert_eq!(delegated_from, Some(approver));
}
...
async fn decide_time_delegation_stamps_the_authority_source() {
    ...
    let outcome = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();
    let (status, _) = step_row(&pool, outcome.request_id, approver).await;
    assert_eq!(status, "pending");
...
async fn resolver_materializes_the_dynamic_chain_once() {
    ...
    let policy = seed_policy(&pool, "leave").await;
    seed_template(&pool, policy, 1, "manager_of_requester", None, None).await;
    seed_template(&pool, policy, 2, "department_head", None, None).await;
    seed_template(&pool, policy, 3, "role", Some(role), None).await;
    seed_template(&pool, policy, 4, "position", Some(position), None).await;
    ...
    let outcome = scoped(&pool, svc.file(filing(Uuid::new_v4(), employee)))
        .await
        .unwrap();
    assert_eq!(outcome.verdict, ApprovalStatus::Pending);
    assert_eq!(count_steps(&pool, outcome.request_id).await, 4);

    // Single-approver kinds carry no ref; multi-holder kinds keep the template id
    // (the identity decide-time semantics key on).
    assert_eq!(
        step_kind_ref(&pool, outcome.request_id, manager).await,
        ("manager_of_requester".into(), None)
    );
    assert_eq!(
        step_kind_ref(&pool, outcome.request_id, head).await,
        ("department_head".into(), None)
    );
    assert_eq!(
        step_kind_ref(&pool, outcome.request_id, role_holder).await,
        ("role".into(), Some(role))
    );
    assert_eq!(
        step_kind_ref(&pool, outcome.request_id, position_holder).await,
...
async fn role_step_materializes_one_row_per_holder() {
    ...
    let h3 = Uuid::new_v4();
    let policy = seed_policy(&pool, "leave").await;
    seed_template(&pool, policy, 1, "role", Some(role), None).await;
    ...
    for h in [h1, h2, h3] {
        assert_eq!(
            step_kind_ref(&pool, outcome.request_id, h).await,
            ("role".into(), Some(role)),
...
async fn any_holder_first_approve_completes_and_skips_siblings() {
    ...
    let verdict = svc
        .decide(decide_as(outcome.request_id, 1, h1, true))
        .await
        .unwrap();
    ...
    // never decided.
    for h in [h2, h3] {
        assert_eq!(step_row(&pool, outcome.request_id, h).await.0, "skipped");
    }
    assert_eq!(step_row(&pool, outcome.request_id, h1).await.0, "approved");

    // A late sibling decide is the usual out-of-turn 409, not a state change.
    let err = svc
        .decide(decide_as(outcome.request_id, 1, h2, true))
        .await
        .unwrap_err();
    assert!(matches!(err, ApprovalsError::OutOfTurn));
    ...
    let verdict = svc
        .decide(decide_as(outcome.request_id, 2, step2, true))
        .await
        .unwrap();
    assert_eq!(verdict, ApprovalStatus::Approved);
}
...
async fn any_holder_reject_fails_the_request() {
    ...
    let verdict = svc
        .decide(decide_as(outcome.request_id, 1, h2, false))
        .await
        .unwrap();
    assert_eq!(verdict, ApprovalStatus::Rejected);
    assert_eq!(step_row(&pool, outcome.request_id, h1).await.0, "skipped");
    assert_eq!(step_row(&pool, outcome.request_id, h2).await.0, "rejected");
}
...
async fn position_steps_are_any_holder_too() {
    ...
    let p2 = Uuid::new_v4();
    let policy = seed_policy(&pool, "leave").await;
    seed_template(&pool, policy, 1, "position", Some(position), None).await;
    ...
    let verdict = svc
        .decide(decide_as(outcome.request_id, 1, p2, true))
        .await
        .unwrap();
    assert_eq!(verdict, ApprovalStatus::Approved);
    assert_eq!(step_row(&pool, outcome.request_id, p1).await.0, "skipped");
    assert_eq!(step_row(&pool, outcome.request_id, p2).await.0, "approved");
}
...
async fn role_with_no_holders_fails_closed() {
    ...
    let err = scoped(&pool, svc.file(filing(Uuid::new_v4(), employee)))
        .await
        .unwrap_err();
    ...
    let n: i64 = sqlx::query_scalar(
        r#"SELECT count(*) FROM approvals.approval_requests WHERE requested_by = $1"#,
    )
    .bind(employee)
    .fetch_one(&pool)
    .await
...
async fn second_live_template_at_one_step_no_is_refused() {
    let (_sequential, pool) = pool().await;
    let policy = seed_policy(&pool, "leave").await;
    seed_template(&pool, policy, 1, "role", Some(Uuid::new_v4()), None).await;
    ...
    let err = pool
        .execute(
            sqlx::query(
                r#"INSERT INTO approvals.approval_step_templates
                       (id, policy_id, step_no, approver_kind, approver_ref,
                        all_of, sla_hours, metadata)
                   VALUES ($1, $2, 1, 'role', $3, NULL, NULL, '{}'::jsonb)"#,
            )
            .bind(Uuid::new_v4())
            .bind(policy)
            .bind(Uuid::new_v4()),
    ...
    // the same discipline the org-scoped one-active-policy index (owned by the
    // composing service's tenancy decorator) imposes one level up.
    sqlx::query(
...
async fn mark_decide_row_truth_reports_a_lost_race() {
    ...
    svc.decide(decide_as(outcome.request_id, 1, h1, true))
        .await
        .unwrap();
...
// ─── 9: filing idempotency, including the concurrent loser ──────────────────
...
async fn concurrent_filings_converge_on_one_request() {
    ...
    assert_eq!(
        count_steps(&pool, id).await,
        2,
        "the quorum materialized exactly once"
    );
}

// ─── 10: withdraw frees the resource for a fresh chain ────────────────────────
...
async fn dynamic_approver_kinds_fail_closed() {
    ...
    // The shipped default resolves nothing — prove it by behavior, not type. The scope
    // is bound so the failure under test is the RESOLVER's, not the seam's: without one
    // the dynamic kind fails closed on the missing company scope first.
...
// ─── 12: the guarded routes behind the org session context ──────────────────
...
async fn decide_ignores_presented_role_refs() {
    ...
    assert_eq!(
        step_row(&pool, outcome.request_id, approver).await.0,
        "pending"
    );
}

// ─── 13: delegation is a principal-verb, not master data ─────────────────────
//
...
// self-service verbs stamping the principal from the session: a body approverId is
...
#[tokio::test]
async fn delegation_verb_stamps_the_session_principal() {
    let (_sequential, pool) = pool().await;
    let approver = Uuid::new_v4();
    let delegate = Uuid::new_v4();
    ...
    // A foreign approverId in the body grants nothing — the row stamps the session's sub.
    let (status, body) = req(
        app.clone(),
        "POST",
    ...
    .await
    .unwrap();
    assert_eq!(row_approver, approver, "the principal is the session's sub");
...
async fn delegation_revoke_discipline_and_post_revoke_semantics() {
    ...
    let live = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();
    let (assigned, inherited) = step_row(&pool, live.request_id, delegate).await;
    ...
    let verdict = svc
        .decide(decide_as(live.request_id, 1, delegate, true))
        .await
        .unwrap();
    assert_eq!(verdict, ApprovalStatus::Approved);

    // A filing AFTER the revoke resolves onto the approver again…
    let fresh = svc.file(filing(Uuid::new_v4(), employee)).await.unwrap();
    let (assigned, inherited) = step_row(&pool, fresh.request_id, approver).await;
    assert_eq!(assigned, "pending");
    assert_eq!(inherited, None);

    // …and the delegate no longer holds decide-time authority over it: 403.
    let err = svc
        .decide(decide_as(fresh.request_id, 1, delegate, true))
        .await
        .unwrap_err();
    assert!(matches!(err, ApprovalsError::NotStepApprover), "{err}");
}

// ─── 14: chain validity is enforced at file time ─────────────────────────────
//
...
async fn policy_without_templates_fails_at_file() {
    let (_sequential, pool) = pool().await;
    let employee = Uuid::new_v4();
    seed_policy(&pool, "leave").await;

    let svc = ApprovalsWriteService::new(pool.clone());
    let err = svc
        .file(filing(Uuid::new_v4(), employee))
        .await
        .unwrap_err();
    assert!(matches!(err, ApprovalsError::InvalidChain(_)), "{err}");
    assert_eq!(err.http_status(), 422);
...
async fn chain_with_a_step_gap_fails_at_file() {
    let (_sequential, pool) = pool().await;
    let policy = seed_policy(&pool, "leave").await;
    seed_template(&pool, policy, 1, "specific_employee", Some(Uuid::new_v4()), None).await;
    seed_template(&pool, policy, 3, "specific_employee", Some(Uuid::new_v4()), None).await;

    let svc = ApprovalsWriteService::new(pool.clone());
    let err = svc
        .file(filing(Uuid::new_v4(), Uuid::new_v4()))
        .await
        .unwrap_err();
    assert!(matches!(err, ApprovalsError::InvalidChain(_)), "{err}");
    assert_eq!(err.http_status(), 422);
...
async fn chain_not_starting_at_one_fails_at_file() {
    let (_sequential, pool) = pool().await;
    let policy = seed_policy(&pool, "leave").await;
    seed_template(&pool, policy, 2, "specific_employee", Some(Uuid::new_v4()), None).await;
...
async fn quorum_sharing_one_delegate_fails_at_file() {
    ...
    seed_delegation(&pool, member_a, shared_delegate, window.0, window.1).await;
    seed_delegation(&pool, member_b, shared_delegate, window.0, window.1).await;
